  ...
  */
  Pos bfs(const Pos& start) {
    ...
    while (!q.empty()) {
        ...
        vector<Pos> neighbors = {act + Dir(0),{act + Dir(1)},
        			                  {act + Dir(2)},{act + Dir(3)},
        			                  {act + Dir(4)},{act + Dir(5)},
        			                  {act + Dir(6)},{act + Dir(7)}};
        for (auto neighbor : neighbors) {
            if (pos_ok(neighbor) and cell(neighbor).type != Rock and not visited[neighbor.i][neighbor.j]) {
                q.push(neighbor);
  ...
  Dir dir_near_pioneer(Pos p){
    Pos go = bfs(p);
    cerr << "---- DFS furyan pos (" << p.i << ", " << p.j << ") atack pioneer pos (" << go.i << ", " << go.j << ")" << endl;
    return Dir(None);
  }
